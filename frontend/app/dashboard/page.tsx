...
export default function Dashboard() {
    ...
    const requestDeletePoll = (e: React.MouseEvent, pollId: string) => {
        e.preventDefault();
        e.stopPropagation();
        setDeleteTarget(pollId);
    };

    const confirmDeletePoll = async () => {
        if (!deleteTarget) return;
        try {
            await axios.delete(`${API_URL}/api/polls/${deleteTarget}`);
            setPolls(polls.filter(p => p.id !== deleteTarget));
                                    ...
                                    <Link href={`/poll/${poll.id}`} key={poll.id} className="group cursor-pointer">
                                        ...
                                        >
                                            <div className="flex justify-between items-start">
                                                ...
                                                <button
                                                    onClick={(e) => requestDeletePoll(e, poll.id)}
